...
int main() {
	...
	for (int i = 0; i < City::Cities; i++) {
		for (int j = 0; j < City::Cities; j++) {

			if(board.paths[cityNames[i]][cityNames[j]])
				cout << cityNames[i] << " -> " << cityNames[j] << endl;
		}
	}*/
	
	cout <<"Pathhhh " << "Sydney->Losanagels:" << board.paths["Sydney"]["LosAngeles"] << endl;
